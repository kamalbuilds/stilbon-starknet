	...
	useEffect(() => {
		async function getUserDetails() {
			try {
				if (user !== undefined) {
					setLoading(true);
					const { data } = await axios.get(`/api/stilbon/users`, {
						params: { uId: user.uid },
					});
	...
	return (
		<div className="min-h-screen flex flex-col relative">
			...
			<InfoModal
				...
				message={
					'Tier upgrade is not implemented yet, if you want to become a PRO user on StilBon please contact StilBonapplication@gmail.com or leave a message on our discord server.'
				}
			/>
			<Footer />
		</div>
